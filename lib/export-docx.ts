...
function cell(text: string, bold = false, shaded = false) {
...
function actionItemsTable(items: MeetingSummary["action_items"]): Table {
...
function buildMeetingChildren(
  ...
  if (datePart) metaParts.push(new TextRun({ text: datePart, size: 20, color: "64748b" }));
  if (datePart && label) metaParts.push(new TextRun({ text: "  ·  ", size: 20, color: "94a3b8" }));
  if (label) metaParts.push(new TextRun({ text: label, size: 20, color: "6366f1", bold: true }));
...
export async function exportToDocx(summary: MeetingSummary, label = "Meeting Summary"): Promise<void> {
...
export async function exportAllToDocx(entries: HistoryEntry[]): Promise<void> {
  const children = entries.flatMap((e, i) =>
    buildMeetingChildren({ ...e.summary, title: e.title }, e.label, i > 0, e.timestamp)
  );
