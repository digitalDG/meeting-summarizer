...
export function formatSummaryAsText(s) {
    const lines = [`# ${s.title}`, "", s.executive_summary];
...
export function formatSummaryAsHtml(s) {
    const parts = [];
...
export function formatSummaryAsEmailText(s) {
    const lines = [s.title.toUpperCase(), "", s.executive_summary];
    if (s.action_items.length > 0) {
        lines.push("", "ACTION ITEMS");
        s.action_items.forEach((a) => {
            let item = `• ${a.task}`;
